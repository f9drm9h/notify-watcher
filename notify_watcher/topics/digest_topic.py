...
buffer into one digest notification and clears it. Registered AFTER the
...
The digest flush itself owns the optional Gemini summary. When configured and
available it sends a short briefing instead of the raw wall of items; when
Gemini fails, times out, or returns nothing it falls back to the standard grouped
list. No event is ever lost to summarization because digest.flush only clears
the buffer after ntfy.push succeeds, and the event log keeps the underlying
items regardless.
...
def run(state: dict) -> dict:
    ...
    # daily run costs no API calls. Gemini summarization happens inside
    # digest.flush, immediately before ntfy.push.
    header = None
